...
class AppLauncher:
    ...
    def launch_local_servers(self) -> Tuple[bool, str]:
        ...
        print("\n=======================================================")
        print("[LAUNCH] LAUNCHING LOCAL DEVELOPMENT SERVERS FOR TESTING")
        print("=======================================================")

        # 1. Check if Maven is available & launch backend
        if self.backend_dir.exists() and (self.backend_dir / "pom.xml").exists():
            print("[BACKEND] Starting Java Spring Boot Backend Server (mvn spring-boot:run)...")
            try:
                ...
                print("   [WARNING] Maven (mvn) not found in system PATH. Cannot auto-start Spring Boot backend.")
        ...
        if self.frontend_dir.exists() and (self.frontend_dir / "package.json").exists():
            print("[FRONTEND] Starting React Frontend UI Server (npm start)...")
            try:
                ...
                print("   [WARNING] NPM (npm) not found in system PATH. Cannot auto-start React UI frontend.")
                logs.append("NPM not found in PATH.")

        # 3. Open Browser or Fallback to Copilot Web Dashboard
        if backend_proc or frontend_proc:
            print("\n[BROWSER] Opening Local Application URLs in Default Browser...")
            time.sleep(2)
            try:
                if backend_proc:
            ...
            print("\n[INFO] Maven/NPM not found in system PATH. Launching Agent Copilot Web Dashboard UI at http://localhost:5000...")
            try:
